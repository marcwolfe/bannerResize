function initHandler() {
  ...
  var m = mkr.makeDC(_width, _height, {
    css: { autoAlpha: 1, pointerEvents: "auto" },
    attr: { id: "main" },
    preload: true,
  });
  //console.log(m);

  m.create("img", { attr: { src: "bg-0.png" }, height: 250 });
  ...
  function loadComplete() {
    activateClickTags();
    if (isiScreenShotHandler(isi)) return;

    initAnims();
  }

  function scrollISI() {
    TweenMax.to(isi, isiScrollTime, {
      scrollTo: { y: "max" },
      ease: Linear.easeNone,
      delay: 0,
      onComplete: function () {
        TweenLite.to(isi, 0.5, { scrollTo: 0, ease: Power2.easeOut });
      },
    });
  }
